---
description: Flutter/Dartの未コミット差分を意図・正確性・設計品質の観点でレビューする
---
...
6. **Flutter/Dart specific checks** — Verify widget lifecycle correctness (`setState`/`mounted` after async gaps, no use of `BuildContext` across `await` without an `if (!mounted) return` guard), disposal of controllers / `StreamSubscription` / `Timer` / `AnimationController`, null-safety soundness (no unjustified `!`), `Future`/error propagation, and `const`/rebuild efficiency.
7. **Mobile/runtime checks** — Verify runtime permission handling (location, background), background/foreground and location-service behavior, Android/iOS platform differences and platform-channel safety, local persistence/migration correctness, and battery/resource impact of polling or location streams.
8. **Repository hygiene checks** — Flag accidental inclusion of build artifacts (`build/`, `.dart_tool/`, `*.apk`/`*.aab`/`*.ipa`), generated files, caches, logs, signing keys (`*.keystore`/`*.jks`/`key.properties`), or secrets/credentials in the diff.
9. **Propose future improvements** — If anything works today but will become a maintenance hazard, suggest a better design.
...
- Include file name and line number(s) for each issue (e.g., `app.py:123`).
